...
def file_to_df(file_name):
    ...
    if file_extension=='.csv':
        df = pd.read_csv(file_name, sep=',', header=0).iloc[:,1:]
...
class ReactionNetwork():
    def __init__(self, database_file):
        ...
        self.df_from_db = db_to_df(database_file)
    ...
    def write_input(self, filename):
        ...
        if file_extension == '.csv':
            self.df_reaction_parameters.to_csv(filename, header=0)
        elif file_extension == '.tsv':
            self.df_reaction_parameters.to_csv(filename, sep='\t', header=0)
        else:
    ...
    def set_corrections(self, net_corrections=None, dft_corrections=None,
                        field_corrections=None, overbinding_corrections=None,
        ...
        if any([self.net_corrections, self.dft_corrections, self.field_corrections,
            ...
            names = ['net_corrections', 'dft_corrections', 'field_corrections',
                    'overbinding_corrections', 'solvation_corrections']
            correction_list = [self.net_corrections, self.dft_corrections, self.field_corrections,
                               self.overbinding_corrections, self.solvation_corrections]
    ...
    def get_corrections(self, filename=''):
        ...
        # merge both
        dfnew = pd.concat([df, df2], sort=False, axis = 1)
        self.df_reaction_parameters = dfnew
        return(True)


    # REACTION SCHEME
    def reaction_scheme(self, df, temperature, pressure, pH, potential):
        ...
        df_param = self.df_reaction_parameters.fillna(0)
        ...
        self.df_react = df_new
        return(True)

    def plot_network(self, show=False):
        self.reaction_scheme(self.df_from_db, potential=self.potential, pH=self.pH,
